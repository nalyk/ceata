...
while (ctx.state.stepCount < ctx.options.maxSteps && !ctx.state.isComplete) {
  ...
  if (stepResult.providerUsed) {
    logger.debug(`🔧 Step executed by: ${stepResult.providerUsed.id} (${stepResult.providerUsed.model})`);
  }
...
*This document reflects the exact current state of Ceata development as of the latest commit. The framework has achieved production readiness across all major components, with 9,000+ lines of TypeScript implementing a complete universal agentic system.*
