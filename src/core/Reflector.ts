...
export interface ReflectionResult {
  ...
  readonly correctedMessage?: ChatMessage;
...
export class Reflector {
   ...
   */
  async review(stepResult: StepResult, ctx: AgentContext): Promise<ReflectionResult | null> {
   ...
   */
  private detectIssues(message: ChatMessage, ctx: AgentContext): string[] {
    const issues: string[] = [];
   ...
   */
  private shouldUseTools(content: string, ctx: AgentContext): boolean {
   ...
   */
  private isComplexQuery(ctx: AgentContext): boolean {
    const lastUserMessage = this.getLastUserMessage(ctx);
    if (!lastUserMessage) return false;
   ...
   */
  private generateCorrection(message: ChatMessage, issues: string[], ctx: AgentContext): ChatMessage | null {
   ...
   */
  private getLastUserMessage(ctx: AgentContext): ChatMessage | null {
