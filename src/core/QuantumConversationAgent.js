...
import { QuantumPlanner } from "./QuantumPlanner.js";
import { Executor } from "./Executor.js";
import { Reflector } from "./Reflector.js";
import { logger } from "./logger.js";
export class QuantumConversationAgent {
    constructor() {
        this.quantumPlanner = new QuantumPlanner();
    ...
    async run(messages, tools, providers, options, providerModels) {
        ...
        while (ctx.state.stepCount < ctx.options.maxSteps && quantumPlan.steps.length > 0) {
            ...
            if (stepResult.providerUsed) {
                logger.debug(`🔧 Quantum Step ${stepCount} executed by: ${stepResult.providerUsed.id}` +
            ...
            if (stepResult.error && stepCount > 2) {
                ...
                if (quantumPlan.alternatives.length > 0) {
                    const bestAlternative = quantumPlan.alternatives.reduce((best, current) => current.reliability > best.reliability ? current : best);
