import { runAgent, AgentOptions } from "../core/AgentRunner.js";
...
(async () => {
  ...
  try {
    ...
    const providers = [
      { p: openRouter, model: "mistralai/mistral-small-3.1-24b-instruct:free", priority: "primary" as const },
    ];
    ...
    const longConversation = [
      { role: "system" as const, content: "You are a helpful assistant with access to an echo tool. Keep responses brief." },
      // Add many user/assistant pairs to simulate a long conversation
      ...Array.from({ length: 15 }, (_, i) => [
        { role: "user" as const, content: `Message ${i + 1}: Hello assistant` },
        { role: "assistant" as const, content: `Response ${i + 1}: Hello! How can I help you?` },
      ]).flat(),
      { role: "user" as const, content: "Use the echo tool to say 'Memory management working!'" },
    ];
