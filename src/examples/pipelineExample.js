...
const echoTool = defineTool({
    ...
    execute: async ({ message, emphasis = "normal" }) => {
        const prefixes = { normal: "Echo: ", loud: "ECHO: ", whisper: "echo: " };
        const result = prefixes[emphasis] + message;
...
(async () => {
    ...
    try {
        ...
        const mathQuery = [
            { role: "system", content: "You are an efficient math assistant. Use tools when needed." },
            { role: "user", content: "Calculate 15 * 8 + 42, then echo the result loudly!" },
        ];
        ...
        const complexQuery = [
            { role: "system", content: "You are an ultra-efficient assistant." },
            { role: "user", content: "I need you to calculate 25 * 4, then echo that result with normal emphasis, then calculate the square of that echoed number." },
        ];
        ...
        const pipelineResult = await pipelineAgent.run(complexQuery, tools, providerGroup, { enableRacing: true, maxSteps: 10 });
