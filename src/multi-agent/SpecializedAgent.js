...
export class SpecializedAgent extends ConversationAgent {
    ...
    async quickAssess(userInput) {
        ...
        const confidence = Math.min((toolMatch * 0.5 + languageMatch * 0.3 + loadCapacity * 0.2), 0.98);
     ...
     */
    async evaluateTask(userInput, context) {
        const toolMatch = await this.calculateToolMatch(userInput);
        const languageMatch = await this.calculateLanguageMatch(userInput);
        const loadCapacity = this.calculateLoadCapacity();
        const timeEstimate = this.estimateExecutionTime(userInput);
        const score = (toolMatch * 0.4 + // Tool availability most important
     ...
     */
    async calculateToolMatch(userInput) {
        ...
        const relevanceScores = await Promise.all(availableTools.map(tool => this.assessToolRelevanceWithLLM(tool, userInput)));
     ...
     */
    async assessToolRelevanceWithLLM(tool, userInput) {
...
Response format: Just the number (e.g., 0.8)`;
     ...
     */
    fallbackToolRelevance(tool, userInput) {
        ...
        const overlap = inputWords.filter(word => descWords.some(descWord => this.areWordsSimilar(word, descWord))).length;
     ...
     */
    async calculateLanguageMatch(userInput) {
...
Response format: Just the number (e.g., 0.9)`;
     ...
     */
    fallbackLanguageMatch(userInput, supportedLangs) {
    ...
    assessComplexity(userInput) {
        ...
        let multiplier = 1.0;
        if (words > 40)
            multiplier += 0.8; // Very long request
        else if (words > 20)
            multiplier += 0.4; // Long request
        if (sentences > 3)
            multiplier += 0.3; // Multiple sentences
        if (hasMultipleTasks)
            multiplier += 0.5; // Multiple tasks
        if (hasQuestions)
            multiplier += 0.2; // Questions add complexity
        return Math.min(3.0, multiplier); // Cap at 3x base time
    }
    /**
     * Calculate final confidence after task execution
     */
    async calculateFinalConfidence(result) {
        // Simple confidence calculation based on result quality
        const hasError = result.messages.some((m) => m.content.toLowerCase().includes('error'));
        const executionTime = result.metrics.duration;
        const expectedTime = this.capabilities.averageResponseTime;
        let confidence = 0.8; // Base confidence
        if (hasError)
            confidence -= 0.3;
        if (executionTime > expectedTime * 2)
            confidence -= 0.2;
        if (executionTime < expectedTime * 0.5)
            confidence += 0.1;
    ...
    recordTaskResult(result) {
        const taskResult = {
            timestamp: new Date(),
            success: !result.messages.some((m) => m.content.toLowerCase().includes('error')),
     ...
     */
    generateReasoning(toolMatch, languageMatch, loadCapacity) {
        const reasons = [];
        if (toolMatch > 0.7) {
     ...
     */
    enhanceWithSpecialization(messages) {
