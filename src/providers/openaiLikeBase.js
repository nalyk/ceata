...
export function createOpenAILikeProvider(options) {
    const { id, apiKey, baseUrl, path, headers = {}, maxTokens, temperature, timeoutMs, defaultStream = false, normalizeToolArgs, errorPrefix, } = options;
    return {
        id,
        supportsTools: true,
        chat(chatOptions) {
            const { model, messages, tools, stream = defaultStream, timeoutMs: overrideTimeout = timeoutMs, } = chatOptions;
            const requestBody = {
                model,
            ...
            return (async () => {
                ...
                if (toolCalls) {
                    toolCalls = toolCalls.map((tc) => fixToolCall(tc));
                }
