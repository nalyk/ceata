...
export function createGoogleOpenAIProvider(apiKey, baseUrl, options) {
    ...
    if (!actualApiKey) {
        throw new Error("Google OpenAI provider requires a non-empty API key. Set GOOGLE_API_KEY or pass it to createGoogleOpenAIProvider().");
    }
    return {
        id: "google-openai",
        supportsTools: true,
        async chat(options) {
            ...
            const response = await postJSON(`${actualBaseUrl}/chat/completions`, {
