...
test('createOpenAIProvider performs chat', async (t) => {
  await withTemp(config.providers.openai, 'apiKey', 'sk-test', async () => {
    ...
    const { createOpenAIProvider } = await import(`../providers/openai.js?${Date.now()}`);
    const provider = createOpenAIProvider();
    const result = await provider.chat({ model: 'gpt-4o-mini', messages: [{ role: 'user', content: 'hello' }] }) as any;
...
test('createGoogleProvider performs chat', async (t) => {
  await withTemp(config.providers.google, 'apiKey', 'g-key', async () => {
    ...
    const result = await provider.chat({ model: 'models/gemini-2.0-flash-thinking-exp', messages: [{ role: 'user', content: 'hi' }] }) as any;
    ...
    await withTemp(config.providers.openrouter, 'apiKey', 'sk-test', async () => {
      await withTemp(config.providers.google, 'apiKey', 'g-key', async () => {
        ...
        providerCapabilities.forEach(({ provider, type, supportsNativeTools, usage }) => {
          assert.equal(provider.supportsTools, supportsNativeTools, 
            `${type} provider tool support should match expected`);
          
          if (usage === 'primary') {
            assert.ok(['openrouter-vanilla', 'google'].includes(provider.id), 
              `${type} provider should be usable as primary`);
          }
        });
      });
    });
...
test('Provider fallback mechanism', async (t) => {
  await withTemp(config.providers.openai, 'apiKey', 'sk-test', async () => {
    ...
    const failingFetchMock = t.mock.method(globalThis as any, 'fetch', async () => {
...
test('Vanilla provider prompt enhancement', async (t) => {
  await withTemp(config.providers.openrouter, 'apiKey', 'sk-test', async () => {
    const enhancedPromptMock = t.mock.method(globalThis as any, 'fetch', async (url: string, options: any) => {
