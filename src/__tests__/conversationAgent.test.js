...
function createMockProvider(id, responses) {
    let callCount = 0;
    return {
        id,
        supportsTools: true,
...
function createMockVanillaProvider(id, responses) {
    let callCount = 0;
    return {
        id,
...
test('ConversationAgent executes basic conversation', async () => {
    ...
    const result = await runAgent([{ role: 'user', content: 'Hello' }], {}, [{ p: provider, model: 'test', priority: 'primary' }]);
...
test('ConversationAgent handles tool calls (classical)', async () => {
    ...
    const result = await runAgent([{ role: 'user', content: 'Use the test tool' }], { test_tool: mockMathTools.test_tool }, [{ p: provider, model: 'test', priority: 'primary' }]);
...
test('ConversationAgent handles vanilla tool calling (free models)', async () => {
    ...
    const result = await runAgent([{ role: 'user', content: 'Calculate the area of a 15×8 rectangle, then divide by 3' }], mockMathTools, [{ p: vanillaProvider, model: 'mistralai/mistral-small-3.2-24b-instruct:free', priority: 'primary' }]);
...
test('ConversationAgent handles provider racing', async () => {
    ...
    const result = await runAgent([{ role: 'user', content: 'Test racing' }], {}, [
...
test('ConversationAgent handles memory management', async () => {
    ...
    const result = await runAgent(longMessages, {}, [{ p: provider, model: 'test', priority: 'primary' }], {
        maxHistoryLength: 10,
...
test('ConversationAgent fallback to secondary providers (cost optimization)', async () => {
    ...
    const result = await runAgent([{ role: 'user', content: 'Test fallback' }], {}, [
...
test('ConversationAgent free-model-first strategy', async () => {
    ...
    const result = await runAgent([{ role: 'user', content: 'Simple request' }], {}, [
...
test('createAgentContext with custom options', () => {
    const ctx = createAgentContext([{ role: 'user', content: 'Test' }], { test_tool: mockMathTools.test_tool }, {
...
test('ConversationAgent backwards compatibility', async () => {
    ...
    const result = await runAgent([{ role: 'user', content: 'Test' }], {}, [{ p: provider, model: 'test', priority: 'primary' }], { maxSteps: 3 });
...
test('ConversationAgent sequential tool execution correctness', async () => {
    ...
    const result = await runAgent([{ role: 'user', content: 'Calculate area of 15×8 rectangle, then divide by 3' }], mockMathTools, [{ p: sequentialProvider, model: 'test-sequential', priority: 'primary' }]);
...
test('ConversationAgent cost tracking', async () => {
    ...
    const result = await runAgent([{ role: 'user', content: 'Test cost tracking' }], {}, [{ p: freeProvider, model: 'mistralai/mistral-small-3.2-24b-instruct:free', priority: 'primary' }]);
...
test('ConversationAgent error recovery', async () => {
    ...
    const result = await runAgent([{ role: 'user', content: 'Test error recovery' }], {}, [{ p: unreliableProvider, model: 'test', priority: 'primary' }], { retryConfig: { maxRetries: 2, baseDelayMs: 10, maxDelayMs: 100, jitter: false } });
