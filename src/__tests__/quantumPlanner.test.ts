...
function createMockQuantumProvider(id: string, responses: string[]): Provider {
  let callCount = 0;
  return {
    id,
...
const quantumTestTools = {
  ...
  getWeather: defineTool({
    ...
    execute: async ({ city }: { city: string }) => `Weather in ${city}: Sunny, 22°C`
  }),
  ...
  readFile: defineTool({
    ...
    execute: async ({ filename }: { filename: string }) => `Contents of ${filename}`
  })
};
...
test('QuantumPlanner analyzes intent correctly', async () => {
  ...
  const intent = await planner.analyzeIntent(
...
test('QuantumPlanner recognizes parallel operations', async () => {
  const planner = new QuantumPlanner();
  const context = createAgentContext(
    [{ role: 'user', content: 'Get weather for London and New York simultaneously' }],
    quantumTestTools,
    { primary: [], fallback: [] }
  );

  const intent = await planner.analyzeIntent(
    'Get weather for London and New York simultaneously',
    context
  );
...
test('QuantumPlanner recognizes conditional operations', async () => {
  const planner = new QuantumPlanner();
  const context = createAgentContext(
    [{ role: 'user', content: 'If the number 25 is greater than 20, multiply it by 2' }],
    quantumTestTools,
    { primary: [], fallback: [] }
  );

  const intent = await planner.analyzeIntent(
    'If the number 25 is greater than 20, multiply it by 2',
    context
  );
...
test('QuantumPlanner performs HTN task decomposition', async () => {
  const planner = new QuantumPlanner();
  const context = createAgentContext(
    [{ role: 'user', content: 'Calculate area then divide by 3' }],
    quantumTestTools,
    { primary: [], fallback: [] }
  );
...
test('QuantumPlanner self-healing: adapts plan on error', async () => {
  ...
  const adaptContext = createAgentContext(
    [{ role: 'user', content: 'Divide safely by non-zero number' }],
    quantumTestTools,
    { primary: [], fallback: [] }
  );
...
test('QuantumPlanner learning from execution', async () => {
  ...
  const learnContext = createAgentContext(
    [{ role: 'user', content: 'Learn from this execution pattern' }],
    quantumTestTools,
    { primary: [], fallback: [] }
  );
...
test('QuantumConversationAgent: Self-healing with provider fallback', async () => {
  const failingQuantumProvider: Provider = {
    id: 'failing-quantum',
    supportsTools: false,
    async chat() {
...
test('QuantumConversationAgent: Memory and learning integration', async () => {
  ...
  for (let i = 0; i < 3; i++) {
    const result = await quantumAgent.run(
      [{ role: 'user', content: `What is ${i+5} times ${i+6}?` }],
      quantumTestTools,
      { primary: [quantumProvider], fallback: [] },
      { maxSteps: 5, providerStrategy: 'smart' }
    );
